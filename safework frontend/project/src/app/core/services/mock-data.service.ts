...
export class MockDataService {
  ...
  private loadAllData(): void {
    ...
    forkJoin({
      ...
      auditLogs: this.http.get<unknown[]>(`${this.apiBase}/users/audit-logs`, headers).pipe(catchError(() => of([]))),
    }).subscribe(result => {
      ...
      this.reports.set([]);
      this.auditLogs.set((result.auditLogs ?? []).map(l => this.mapAuditLog(l)));
  ...
  private mapAuditLog(raw: unknown): AuditLog {
    const r = raw as Record<string, unknown>;
    ...
    return {
      ...
      action: String(r['userAuditAction'] ?? 'UNKNOWN'),
      resource: String(r['userAuditResource'] ?? 'N/A'),
      timestamp: String(r['userAuditTimeStamp'] ?? new Date().toISOString()),
    };
  }
  ...
  private toUiTrainingStatus(status: string): Training['status'] {
    ...
    return map[status.toUpperCase()] ?? 'NOT_STARTED';
  }
  ...
  private generateMockAuditLogs(): AuditLog[] {
    return []; // No longer used, fetching from DB
  }
}
